...
public class StoneTile extends Tile
{
	...
	public void render(Screen screen, Level level, int x, int y)
	{
		...
		if (!u && !l)
		{
			...
			else screen.render(x * 16 + 0, y * 16 + 0, 8 + 1 * 32, transitionColor, 0);
		}
		else
		{
		...
		if (!u && !r)
		{
			...
			else screen.render(x * 16 + 8, y * 16 + 0, 7 + 1 * 32, transitionColor, 0);
		}
		else
		{
		...
		if (!d && !l)
		{
			...
			else screen.render(x * 16 + 0, y * 16 + 8, 8 + 0 * 32, transitionColor, 0);
		}
		else
		{
		...
		if (!d && !r)
		{
			...
			else screen.render(x * 16 + 8, y * 16 + 8, 7 + 0 * 32, transitionColor, 0);
		}
		else
		{
